...
════════════════════════════════════════════════════════════════
👤 CUENTA ADMINISTRADORA DEL PANEL
════════════════════════════════════════════════════════════════
• Email:         $EMAIL
• Usuario:       $USERNAME
...
════════════════════════════════════════════════════════════════
🗄️ BASE DE DATOS — USUARIO DE LA APLICACIÓN (PANEL)
════════════════════════════════════════════════════════════════
• Database:      panel
• Host:          127.0.0.1:3306
...
════════════════════════════════════════════════════════════════
🛠️ BASE DE DATOS — USUARIO ADMINISTRATIVO (HOST)
════════════════════════════════════════════════════════════════
...
• MySQL Workbench
• Backups
• Administración manual de la DB

════════════════════════════════════════════════════════════════
📂 ARCHIVOS IMPORTANTES
════════════════════════════════════════════════════════════════
• Configuración Panel:     /var/www/pterodactyl/.env
• Archivo credenciales:   $CREDENTIALS_FILE
• Enlace rápido:          /var/www/pterodactyl/credenciales.txt
• Config Nginx:           /etc/nginx/sites-enabled/pterodactyl.conf
• Servicio Panel:         /etc/systemd/system/pteroq.service
...
• Verificar base de datos:
  mysql -u pterodactyl -p'$DBPASSWORD' -e "SHOW DATABASES;"

• Backup de la base de datos:
  mysqldump -u pterodactyluser -p'$DBPASSWORDHOST' panel > panel_backup.sql

════════════════════════════════════════════════════════════════
⚠️ NOTAS DE SEGURIDAD IMPORTANTES
════════════════════════════════════════════════════════════════
1. Este archivo contiene CREDENCIALES REALES
2. Guárdalo en un lugar seguro
3. Cambia las contraseñas tras el primer login
4. Elimina este archivo si ya no lo necesitas
5. El archivo .env también contiene datos sensibles
...
finish() {
  ...
  echo "================================================"
  echo "   🦊 SERVERS — INSTALACIÓN COMPLETADA"
  echo "================================================"
  echo ""
  echo "✔ Pterodactyl Panel se instaló correctamente"
  echo ""
  ...
  echo ""
  echo "⚠️  Importante:"
  echo "   - Guarda el archivo de credenciales en un lugar seguro"
...
instalar_wings() {
  ...
  echo ""
  read -rp "¿Continuar con la instalación de Wings? (s/N): " confirm
  [[ ! "$confirm" =~ ^[SsYy]$ ]] && log_info "Instalación cancelada." && return

  if [ "$SSL_MODE" -eq 1 ]; then
    read -rp "Introduce el subdominio (ej: wings.midominio.com): " WINGS_DOMAIN
  ...
  if command -v ufw &>/dev/null; then
    for p in "${PORTS_TO_OPEN[@]}"; do ufw allow "$p/tcp"; done
    ufw --force enable
  else
...
EOF
  ...
  # ==============================
  echo ""
  echo "Pega el comando generado en el panel de Pterodactyl:"
  read -rp "> " WINGS_TOKEN_COMMAND
