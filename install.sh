...
if [ -d "/var/www/pterodactyl" ]; then
    echo ""
    echo "[!] ADVERTENCIA: ¡Pterodactyl ya está instalado!"
    echo ""
    echo "Elige una de las siguientes opciones:"
    echo "  1) Desinstalar Pterodactyl automáticamente (elimina todo lo relacionado con el panel)"
    echo "  2) Continuar de todos modos (¡puede causar errores!)"
    echo "  3) Cancelar instalación"
    echo ""

    while true; do
        read -rp "Ingresa tu opción (1/2/3): " CHOICE
        case "$CHOICE" in
            1)
                echo "[!] Ejecutando desinstalación automática..."
                if [ -d "/var/www/pterodactyl" ]; then
                    rm -rf /var/www/pterodactyl || { echo "Error: No se pudieron eliminar los archivos del panel."; exit 1; }
                else
                    echo "No se encontraron archivos del panel, omitiendo eliminación."
                fi
                ...
                mariadb -u root -e "DROP DATABASE IF EXISTS \`${DB_NAME}\`;" || { echo "No se pudo eliminar la base de datos '${DB_NAME}'."; exit 1; }
        
                for user in "${USERS[@]}"; do
                    mariadb -u root -e "DROP USER IF EXISTS '${user}'@'127.0.0.1';" || { echo "No se pudo eliminar el usuario '${user}'."; exit 1; }
                done
                break
                ;;
            2)
            ...
            3)
                ...
                ;;
            *)
                echo "[!] Selección inválida. Por favor selecciona 1, 2 o 3."
                ;;
        esac
    done
fi
...
panel_input() {
    ...
    while :; do
        ...
        if [ -z "$input" ]; then
            ...
            echo "[!] La entrada no puede tener más de $max_length caracteres."
        elif [[ "$input" =~ [æøåÆØÅ] ]]; then
            echo "[!] Se han detectado caracteres no válidos. Solo se permiten letras, números y símbolos comunes."
        else
            eval "$var_name=\"$input\""
            break
        fi
...
panel_validate_ssl_files() {
    ...
    if [ ! -f "$cert_path" ]; then
        echo "[!] Error: El archivo del certificado (fullchain) no existe en $cert_path."
        exit 1
    fi
    if [ ! -f "$key_path" ]; then
        echo "[!] Error: El archivo de la clave privada no existe en $key_path."
        exit 1
    fi

    if ! openssl x509 -in "$cert_path" -noout; then
        echo "[!] Error: $cert_path no es un certificado SSL válido."
        exit 1
    fi

    if ! openssl rsa -in "$key_path" -check -noout; then
        echo "[!] Error: $key_path no es una clave privada válida."
        exit 1
    fi
    
    echo "[+] Los archivos SSL son válidos."
    panel_email
}
...
panel_webserver() {
    send_summary
    echo "[!] Selecciona el servidor web"
    ...
    case $option in
        ...
        *) 
            echo ""
            echo "Por favor, introduce una opción válida (1 o 2)"
            panel_webserver
            ;;
    esac
}
...
panel_fqdn() {
    send_summary
    echo "[!] Por favor introduce el FQDN. Accederás al panel usando este dominio."
    echo "[!] Ejemplo: panel.tudominio.com"
    read -r FQDN
    FQDN=$(echo "$FQDN" | tr '[:upper:]' '[:lower:]')
    [ -z "$FQDN" ] && echo "El FQDN no puede estar vacío." && return 1

    if [[ "$FQDN" == "localhost" || "$FQDN" == "127.0.0.1" ]]; then
        echo "[!] No puedes usar 'localhost' o '127.0.0.1' como FQDN."
        return 1
    fi

    if [[ "$FQDN" =~ ^([0-9]{1,3}\.){3}[0-9]{1,3}$ ]]; then
        echo "[!] Has introducido una dirección IPv4, no un dominio."
        echo "[!] Los certificados SSL no funcionan con direcciones IP."
        SSLSTATUS=false
    else
        if ! [[ "$FQDN" =~ ^[a-z0-9.-]+$ ]]; then
            echo "[!] Se detectaron caracteres inválidos en el FQDN."
            echo "[!] Usa solo letras minúsculas, números, puntos y guiones."
            return 1
        fi
    fi

    echo ""
    echo "[+] Obteniendo IPv4 pública..."
    ...
    if [ -z "$IP_CHECK" ]; then
        echo "[ADVERTENCIA] El resolver principal falló. Probando el secundario..."
        IP_CHECK=$(curl -4 -s --max-time 5 "$SECONDARY_URL")
    fi

    if [ -z "$IP_CHECK" ]; then
        echo "[ERROR] No se pudo obtener la IPv4 pública."
        return 1
    else
        echo "[+] IPv4 pública detectada: $IP_CHECK"
    fi
    ...
    if [ -z "$DOMAIN_PANELCHECK" ]; then
        echo "[!] No se pudo resolver $FQDN a una IP."
        echo "[!] Si estás ejecutando esto localmente o usando solo IP, puedes ignorarlo."
        echo "[!] Continuando en 10 segundos... Presiona CTRL+C para cancelar."
        sleep 10
    fi

    echo "[+] $FQDN resuelve a: $DOMAIN_PANELCHECK"
    panel_ssl
}

panel_ssl() {
    send_summary
    echo "[!] ¿Deseas usar SSL para tu panel? (Recomendado) (S/N)"
    echo "[!] SSL es recomendado para cualquier panel."
    while :; do
        read -r SSL_CONFIRM
        if [[ "$SSL_CONFIRM" =~ [SsYy] ]]; then
            SSLSTATUS=true
            panel_ssltype
            break
            ...
            echo "[!] Entrada inválida, por favor introduce S o N."
        fi
    done
}
...
panel_ssltype() {
    send_summary
    echo "[!] Selecciona el tipo de SSL"
    echo "    (1) Let's Encrypt (recomendado)"
    echo "        Se te pedirá aceptar los Términos de Servicio."
    echo "    (2) Certificado personalizado"
    echo "    Ingresa 1 o 2"
    read -r option
    case $option in
        1)
        ...
        2)
            CUSTOMSSL=true
            panel_input "Introduce la ruta del certificado SSL (el archivo debe existir)." "CERTIFICATEPATH" 250
            panel_input "Introduce la ruta de la clave privada (el archivo debe existir)." "PRIVATEKEYPATH" 250
...
panel_email() {
    send_summary
    while true; do
        panel_input "[!] Introduce tu correo electrónico (usado para el panel y SSL)." "EMAIL" 50
        EMAIL="${EMAIL,,}"
        EMAIL="${EMAIL:0:32}"
        ...
        if [[ "$EMAIL" =~ ^[a-z0-9._%-]+@[a-z0-9.-]+\.[a-z]{2,}$ ]]; then
            ...
            echo "[!] Formato de correo inválido o caracteres no permitidos."
            echo "[!] Usa solo letras minúsculas, números y los símbolos . _ - @"
            read -p "¿Deseas intentarlo nuevamente? (S/N): " answer
            [[ ! "$answer" =~ ^[SsYy]$ ]] && exit 1
        fi
    done
    panel_admin_setup
...
panel_admin_setup() {
    send_summary
    panel_input "[!] Introduce el nombre de usuario para el administrador del panel." "USERNAME" 20
    panel_input "[!] Introduce el nombre del administrador." "FIRSTNAME" 30
    panel_input "[!] Introduce el apellido del administrador." "LASTNAME" 30

    while :; do
        panel_input "[!] Introduce la contraseña para el administrador del panel." "USERPASSWORD" 64 true
        panel_input "[!] Confirma la contraseña para el administrador del panel." "USERPASSWORDCONFIRM" 64 true
    ...
    send_summary
    echo "[!] Iniciando instalación de Pterodactyl..."
    sleep 2
    panel_installation
}
